...
window.Uploader = MultipleUploader;
}(jQuery));


/*        new Uploader({
            trigger: $(".f-btn"),
            action:'http://127.0.0.1/TEAM4/path/upload/index.php',
            data: {'upload': 'chat_'+(new Date()/1000>>0),'type':1},
        }).success(function(res) {
                    console.log(res)
                    if(res){
                        layer.msg("上传成功");
                        // eme.addClass("fadeout-left");
                        // setTimeout(function(){eme.remove()},1000);
                    }
                });*/