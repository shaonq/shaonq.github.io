...
(function () {
    var util = {
        /*组件事件冒泡*/ stope: function (e) {
            e = e || window.event;
            e.stopPropagation ? e.stopPropagation() : e.cancelBubble = true;
        } /*在焦点处插入内容*/, focusInsert: function (obj, str) {
    ...
    var global = {
        faces: {
            ...
            var mod = {
                picture: function (editor, self) { /*插入图片*/
                    ...
                    }, op = {trigger: $(self), action: 'http://127.0.0.1:3000/upload', progress: progress};
                    nui.upload(op, callback)
        ...
        , tpl: {
            text: function (text, isThis) {
                ...
                if (global.isNull(text))return nui.tpl('\
                <li class="{{d.isThis}}">\
                    <div class="chat-main-user">\
                        <img src="{{d.userImg}}">\
                        <cite> {{d.user}} <i>{{d.time}}</i></cite>\
                    </div>\
                    <div class="chat-main-text">{{d.text}}</div>\
                </li>', {
                    text: text,
    ...
    var socket, fn = function () {
        ...
        socket.on('connect', function () {
            nui.log("WebSocket connect", "info");
    ...
    (typeof io === "undefined") ? nui.loadJs("https://cdn.bootcss.com/socket.io/1.7.3/socket.io.min.js", fn) : fn();
    var send = function () {
