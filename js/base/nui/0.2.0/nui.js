    ...
    global.init = function ($) {
        ...
        global.upload = function (t) {
            ...
            e.prototype.init = function () {
                ...
                if (a) {
                    if (c && !RegExp("\\w\\.(" + c + ")$", "i").test(escape(a)))return o.error("不支持该文件格式"), t.value = "";
        ...
        global.open = function (op) {
            ...
            layer.offset = function () {
                ...
                if (config.type === 4) {  //dropdown
                    ...
                    that.css({
                        top: el.offset().top + el.outerHeight() + 4,
        ...
        global.date = function () {
            ...
            return function (conf, callback) {
                ...
                input.on(ev, function () {
                    var _a = global.get.check.date(conf.min);
                    var _b = global.get.check.date(conf.max);
                    if (_a) {
                        conf.min = g.date.now();
                    }
                    if (_b) {
                    ...
                    global.open({
                        ...
                        success: function (_, index) {
                            ...
                            var setDate = function (y, m, d) {
                                bd.html(g.table);
                                g.viewDate.call(_this, y, --m, d, conf.max !== '2099-12-31' ? MaxMin : void 0)
                            };
        ...
        global.anchor = function () {
            var a, b, c, d = this.getAttribute("data-anchor");
            return a = $(d),
                b = a.length ? a.offset().top - 80 : 0, c = b / 9 > 300 ?
                b / 9 : 300,
                global.animate(document.body.scrollTop, b, function (value) {
        ...
        global.renderData = function (type, that, before) {
            ...
            switch (type) {
                ...
                case 'date':
                    $("[" + before + "]", that).each(function () {
                        global.date({elem: this, min: this.getAttribute(before)})
                    });
                    break;
            }
            return global
